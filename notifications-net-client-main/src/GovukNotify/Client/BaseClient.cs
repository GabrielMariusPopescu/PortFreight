...
using GovukNotify.Authentication;
using Newtonsoft.Json;
using Notify.Exceptions;
using Notify.Interfaces;
using Notify.Models.Responses;

namespace GovukNotify.Client;
...
public class BaseClient : IBaseClient
{
    ...
    public BaseClient(IHttpClient client, string apiKey, string baseUrl = "https://api.notifications.service.gov.uk/")
    {
        ...
        var version = typeof(BaseClient).GetTypeInfo().Assembly.GetName().Version;
        if (version == null)
            return;
    ...
    public async Task<string> MakeRequest(string url, HttpMethod method, HttpContent content = null)
    {
        ...
        if (!response.IsSuccessStatusCode) HandleHTTPErrors(response, responseContent);
    ...
    public Uri ValidateBaseUri(string baseUrl)
    {
        ...
        if (!result) throw new NotifyAuthException("Invalid URL provided");
    ...
    private async Task<HttpResponseMessage> SendRequest(string url, HttpMethod method, HttpContent content)
    {
        ...
        var notifyToken = Authenticator.CreateToken(apiKey, serviceId);
        ...
        catch (AggregateException ae)
        {
            ae.Handle(x =>
            {
                if (x is HttpRequestException) throw new NotifyClientException(x.InnerException.Message);
                throw x;
            });
            throw ae.Flatten();
        }
    ...
    private void HandleHTTPErrors(HttpResponseMessage response, string errorResponseContent)
    {
        try
        {
            ...
            throw new NotifyClientException("Status code {0}. The following errors occured {1}",
                errorResponse.getStatusCode(), errorResponse.getErrorsAsJson());
        }
        catch (Exception ex)
        {
            throw new NotifyClientException("Status code {0}. Error: {1}, Exception: {2}",
                response.StatusCode.GetHashCode(), errorResponseContent, ex.Message);
        }
    }
}