...
public class NotificationClient : BaseClient, INotificationClient, IAsyncNotificationClient
{
        ...
        string olderThanId = "", bool includeSpreadsheetUploads = false)
    {
        ...
        if (!string.IsNullOrWhiteSpace(templateType)) query.Add("template_type", templateType);

        if (!string.IsNullOrWhiteSpace(status)) query.Add("status", status);

        if (!string.IsNullOrWhiteSpace(reference)) query.Add("reference", reference);

        if (!string.IsNullOrWhiteSpace(olderThanId)) query.Add("older_than", olderThanId);

        if (includeSpreadsheetUploads) query.Add("include_jobs", "True");
        ...
        string smsSenderId = null)
    {
        ...
        if (smsSenderId != null) o.Add(new JProperty("sms_sender_id", smsSenderId));
        ...
        string emailReplyToId = null, string oneClickUnsubscribeURL = null)
    {
        ...
        if (emailReplyToId != null) o.Add(new JProperty("email_reply_to_id", emailReplyToId));

        if (oneClickUnsubscribeURL != null) o.Add(new JProperty("one_click_unsubscribe_url", oneClickUnsubscribeURL));
        ...
        string clientReference = null)
    {
        ...
        var response = await POST(SEND_LETTER_NOTIFICATION_URL, o.ToString(Formatting.None)).ConfigureAwait(false);
        ...
        string postage = null)
    {
        var requestParams = new JObject
        {
            ...
            { "content", Convert.ToBase64String(pdfContents) }
        };

        if (postage != null) requestParams.Add(new JProperty("postage", postage));

        var response = await POST(SEND_LETTER_NOTIFICATION_URL, requestParams.ToString(Formatting.None))
        ...
        Dictionary<string, dynamic> personalisation = null)
    {
        ...
        var response = await POST(url, o.ToString(Formatting.None)).ConfigureAwait(false);
    ...
    public SmsNotificationResponse SendSms(string mobileNumber, string templateId,
        Dictionary<string, dynamic> personalisation = null, string clientReference = null, string smsSenderId = null)
    {
        try
        {
    ...
    public EmailNotificationResponse SendEmail(string emailAddress, string templateId,
        Dictionary<string, dynamic> personalisation = null, string clientReference = null, string emailReplyToId = null,
    ...
    {
        if (documentContents.Length > 2 * 1024 * 1024) throw new ArgumentException("File is larger than 2MB");
        return new JObject
        {
            { "file", Convert.ToBase64String(documentContents) },
            { "filename", filename },
        ...
        string clientReference = null)
    {
        ...
        if (personalisation != null) personalisationJson = JObject.FromObject(personalisation);
        ...
        if (clientReference != null) o.Add("reference", clientReference);
