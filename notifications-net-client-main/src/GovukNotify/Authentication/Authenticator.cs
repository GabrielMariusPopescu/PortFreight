...
namespace GovukNotify.Authentication;
...
public static class Authenticator
{
    ...
    public static IDictionary<string, object> DecodeToken(string token, string secret)
    {
        try
        {
            ...
            var jsonPayload = decoder.DecodeToObject<IDictionary<string, object>>(token, secret, true);
    ...
    private static void ValidateGuids(string[] stringGuids)
    {
        if (stringGuids == null)
            return;

        if (stringGuids.Any(stringGuid => !Guid.TryParse(stringGuid, out _)))
